...
# The improved static estimation function counts the number of possible mills each player could form in addition to the number of pieces and number of possible moves. This would make the AI player "smarter" by taking into account not just the quantity of pieces and moves, but also the quality of the board position in terms of potential mills.
def staticEstimationOpening(board):
    ...
    numWhiteMills = sum(closeMill(i, board) for i in range(len(board)) if board[i] == 'W')
    numBlackMills = sum(closeMill(i, board) for i in range(len(board)) if board[i] == 'B')

    return numWhitePieces - numBlackPieces + 2 * (numWhiteMills - numBlackMills)
################
