...
#     whiteMoves = generateMovesMidgameEndgame(board)
#     numWhiteMoves = len(whiteMoves)
...
#         return -10000
#     else:
#         # Also count the number of potential mills
#         numPotentialMills = countPotentialMills(board, W)
#         return 1000 * (numWhitePieces - numBlackPieces) + numWhiteMoves + 100 * numPotentialMills


# def countPotentialMills(board, player):
#     potentialMills = 0
#     for idx, pos in enumerate(board):
#         if pos == player:
#             # Check if the position is part of a potential mill
#             if isPotentialMill(board, idx):
#                 potentialMills += 1
#     return potentialMills


# def isPotentialMill(board, idx):
#     millCombinations = getMillCombinations(idx)
#     for combination in millCombinations:
#         # If all positions in the combination have the same piece or are empty, it's a potential mill
#         if all(board[i] == board[idx] or board[i] == x for i in combination):
#             return True
#     return False

# def getMillCombinations(idx):
#     # A predefined list of all possible mill combinations on the board.
#     millCombinations = [
#         [0, 6, 18],
#         [1, 11, 20],
#         [2, 7, 15],
...
#     return [combination for combination in millCombinations if idx in combination]
...
#############
def countPotentialMills(board, player):
    """Counts the number of potential mills for the given player."""
    count = 0
    mills = {
        0: [[6, 18]],
        1: [[11, 20]],
    ...
    for idx, pos in enumerate(board):
        if pos == player:
            for mill in mills.get(idx, []):
                if board[mill[0]] == player and board[mill[1]] == 'x':
                    count += 1
                if board[mill[1]] == player and board[mill[0]] == 'x':
                    count += 1
    return count
...
def staticEstimationOpening(board):
    ...
    return (numWhitePieces - numBlackPieces) + numWhitePotentialMills
...
def staticEstimationMidgameEndgame(board):
    ...
    if numBlackPieces <= 2: 
        ...
        return (1000 * (numWhitePieces - numBlackPieces) - numBlackMoves) + numWhitePotentialMills
