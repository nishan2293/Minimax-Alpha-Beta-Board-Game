...
def minimaxGame(position, depth, isMax, alpha, beta):
    # Base case
    if depth == 0:
    ...
    if isMax:
        ...
        for pos in board_positions:
            evaluation, _ = minimaxGame(pos, depth-1, False, alpha, beta)
        ...
        minPosition = None
        for pos in board_positions:
            evaluation, _ = minimaxGame(pos, depth-1, True, alpha, beta)
...
def main():
    ...
    # Get the best position using minimax with alpha-beta pruning
    estimate, best_position = minimaxGame(position, depth, True, -INF, INF)
    ...
    print(f"AlphaBeta-MINIMAX estimate: {estimate}")
